...
fn main() {
    ...
    let c = cbindgen::Builder::new()
        ...
        .with_header(format!("/* libfil_secp256k1 Header Version {} */", VERSION))
    ...
    match c {
        Ok(res) => {
            res.write_to_file(target_path.join("libfil_secp256k1.h"));
        }
        Err(err) => {
    ...
    let mut pc_file = File::create(target_path.join("libfil_secp256k1.pc"))
        .expect("unable to generate .pc file: {:?}");

    write!(
        pc_file,
        "Name: Filecoin Secp256k1
Version: {version}
Description: Filecoin Secp256k1 library
Libs: {libs}
",
